...
# WARNING 1: The specified values of the niters parameter are too small! To get reliable results,
# use the default or higher values of the niters parameter.
# WARNING 2: To avoid potential issues with parallel computing on CRAN, we set num.threads to 1, 
# by default it is set to half of the available CPU cores, which speeds up computation.
vimp_def <- shadow_vimp(data = data_alz, outcome_var = "diagnosis", niters = c(20, 30, 50), 
...
````{r plot_base_starts,  eval=TRUE, echo=FALSE}
...
# set.seed(786)
# vimp_def <- shadow_vimp(data = data_alz, outcome_var = "diagnosis", 
# niters = c(20, 30, 50), num.threads = global_num_threads)
...
vimp_fdr <- shadow_vimp(data = data_alz, outcome_var = "diagnosis",
                        niters = c(20, 30, 50), to_show = "FDR", 
                        num.threads = global_num_threads)
vimp_fdr

# Show only unadjusted p-values
vimp_unadjusted <- shadow_vimp(data = data_alz, outcome_var = "diagnosis", 
                               niters = c(20, 30, 50), to_show = "unadjusted",
                               num.threads = global_num_threads)
vimp_unadjusted
```
...
````{r per_variable}
vimp_per_variable <- shadow_vimp(data = data_alz, outcome_var = "diagnosis", 
                                 niters = c(20, 30, 50), method = "per_variable",
                                 num.threads = global_num_threads)
...
# vimp_def <- shadow_vimp(data = data_alz, outcome_var = "diagnosis", niters = c(20, 30, 50), num.threads = global_num_threads)
vimp_def
````
...
````{r plot_base,  eval=TRUE, echo=FALSE}
plot_vimps(shadow_vimp_out = vimp_def)
````
...
````{r plot_txt_size,  eval=TRUE, echo=FALSE}
...
````{r filter_vars,  eval=TRUE, echo=FALSE}
...
````{r legend_position,  eval=TRUE, echo=FALSE}
...
```{r p_val_labels,  eval=TRUE, echo=FALSE}
# No p-values labels
...
```{r no_helper_plot,  eval=TRUE, echo=FALSE}
...
```{r change_colors,  eval=TRUE, echo=FALSE}
plot_vimps(
...
```{r other_options,  eval=TRUE, echo=FALSE}
...
````{r plot_per_var2,  eval=TRUE, echo=FALSE}
...
if (is_interactive) {
  # When interactive - set num_cores to a selected value
  # Here we want to use 4 cores or if 4 cores are not available, then the number of available cores
  num_cores <- min(parallel::detectCores(), 4)
  vimp_parallel1 <- shadow_vimp(data = data_alz, outcome_var = "diagnosis",
                                niters = c(20, 30, 50), num.threads = num_cores)
} else {
  # When non-interactive - run with 1 thread
  vimp_parallel1 <- shadow_vimp(data = data_alz, outcome_var = "diagnosis",
                                niters = c(20, 30, 50), num.threads = 1)
}
````
