...
1. For *p* covariates included in the original data set, create *p* r-shadow variables. R-shadow variables are uninformative copies of all predictors added to the dataset for reference. They are created by permuting rows of copied original covariates, which effectively breaks any association with the outcome variable (see the figure below for a visualisation of the permutation process). This lack of association makes the R-shadow variables uninformative and allows them to act as a reference for comparison when assessing the importance of the original predictors. Repeat the permutation `nsim` times. Each permutation will result in a new set of r-shadow variables. At each step, the random forest model is run and new VIMPs are calculated. After `nsim` iterations, the algorithm generates a data frame with *nsim* rows and *2p* columns containing the VIMP values for each original variable, along with the corresponding r-shadow variables. The purpose of these iterations is to generate a distribution of VIMPs under the null hypothesis. The VIMP values for the r-shadow variables vary between iterations due to the permutation process, while the VIMPs of the original covariates vary due to the inherent randomness of the random forest algorithm.
...
2. For each of the `nsim` permutations, recompute the VIMP of the original and r-shadow covariates. As a result, you will get the following sets:
  ...
  * $VI_{j}$ - **set of VIMPs of the original variable *j* across `nsim` iterations**,
...
where $\widetilde{VI}_{j}^{(s)}$ is a set of standardised r-shadow VIMPs:

$$
\widetilde{VI}_{j}^{(s)} = \frac{VI_{j}^{(s)} - \text{mean}(VI_{j}^{(s)})}{\text{sd}(VI_{j}^{(s)})}
$$
and $\widetilde{VI}_{j}$ is a "standardised" VIMP of the original covariate, corrected using the mean and standard deviation of its corresponding r-shadow:
...
**My comment of loading `AppliedPredictiveModeling` package: The `AppliedPredictiveModeling` package is used only in the `shadowVIMP` vignette. Using `library(AppliedPredictiveModeling)` would add it to the Imports field of DESCRIPTION, requiring all users to install it, even if they don't want to build the vignette. My approach ensures the package is only listed in Suggests, making it optional and needed only for building the vignette. To the best of my knowledge, this solution aligns with CRAN policies and best practices.**
