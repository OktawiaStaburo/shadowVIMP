...
### High-dimensional data extension 1: pooled p-value
...
````{r parallel2}
# Technical checks 
# Detect if running on CRAN or non-interactive environments
is_cran <- !interactive() || Sys.getenv("NOT_CRAN") != "true"

# Limit to 1 core on CRAN; otherwise, use available cores - 2
num_cores_parallel <- if (is_cran) 1 else round(parallel::detectCores()/2)

vimp_parallel2 <- vim_perm_sim_wrapper(nsims = c(20,30,50), entire_data = data_sub, outcome_var = "diagnosis", num_cores_parallel = num_cores_parallel)
````
